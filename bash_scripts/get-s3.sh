...
check_staging () {
  if [ "$CHECK_STAGING" != "$STAGING_LATEST" ]; then
    ...
    mkdir -p untar-stage && tar -xzf $STAGING_FILE_PATH/$STAGING_LATEST -C $_
...
if [ "$CHECK_MASTER" != "$MASTER_LATEST" ]; then
    ...
    #Unzip into /var/www (For future use) 
    mkdir -p untar-master && tar -xzf $MASTER_FILE_PATH/$MASTER_LATEST -C $_
